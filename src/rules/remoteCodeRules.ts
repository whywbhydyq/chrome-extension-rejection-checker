...
function scanJs(file: VirtualFile): Finding[] {
  ...
  const patterns: Array<{ pattern: RegExp; title: string; reason: string }> = [
    ...
    { pattern: /(?:\bsrc|\.src|\bhref|\.href|\bdata|\.data)\s*=\s*["']https?:\/\/[^"']+\.(?:js|mjs)(?:[?#][^"']*)?["']/gi, title: 'Remote JavaScript URL assignment found', reason: 'The code assigns a remote JavaScript URL to a property that may load executable code.' },
    { pattern: /setAttribute\s*\(\s*["'](?:src|href|data)["']\s*,\s*["']https?:\/\/[^"']+\.(?:js|mjs)(?:[?#][^"']*)?["']\s*\)/gi, title: 'Remote JavaScript setAttribute found', reason: 'The code sets a remote JavaScript URL on an element attribute that may load executable code.' },
  ]
