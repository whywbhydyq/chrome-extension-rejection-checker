...
export function runIconRules(context: ScannerContext): Finding[] {
  ...
  for (const [size, value] of Object.entries(icons)) {
    ...
    const normalizedIconPath = normalizeManifestResourcePath(value)
    const iconFile = !normalizedIconPath.hadTraversal && !normalizedIconPath.hadLeadingSlash ? context.files.get(normalizedIconPath.path) : undefined
    if (!iconFile) {
