...
export function App() {
  ...
  async function scan(file: File) {
    ...
    try {
      const { readExtensionZip } = await import('../core/zipReader')
  ...
  return (
    <main className="min-h-screen bg-slate-50 px-4 py-8 text-slate-950 sm:py-10">
      <section className="mx-auto max-w-6xl">
        ...
        <section className="grid items-stretch gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(360px,0.9fr)]" aria-label="Local ZIP scanner workbench">
          <div className="space-y-4">
            <div className="rounded-3xl bg-slate-950 p-5 text-white shadow-sm">
              <p className="text-xs font-bold uppercase tracking-widest text-slate-300">Local Manifest V3 preflight</p>
              <h1 className="mt-2 text-3xl font-black tracking-tight">Chrome Extension Rejection Checker</h1>
              <p className="mt-2 text-sm leading-6 text-slate-300">Choose the exact production ZIP you plan to submit. The package stays in this browser.</p>
            </div>
        ...
        <section className="mt-10" aria-label="Scanner explanation">
          <HeroSection />
        </section>
