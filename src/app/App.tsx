...
import type { Finding, ScanReport, Severity } from '../core/types'
...
function FindingCard({ finding, index }: { finding: Finding; index: number }) {
  return (
    <article className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
      <p className="text-xs font-bold uppercase tracking-wide text-slate-500">{finding.severity} · {finding.ruleId} · #{index + 1}</p>
...
function FindingGroups({ report }: { report: ScanReport }) {
      ...
      {severityOrder.map((severity) => {
        ...
        return (
          <section key={severity} className="space-y-4">
            ...
            {findings.map((finding, index) => <FindingCard key={`${finding.ruleId}-${finding.file ?? 'global'}-${index}`} finding={finding} index={index} />)}
          </section>
        )
      })}
    </div>
